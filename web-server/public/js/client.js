...
function initUserList(data) {
	...
	for(var i = 0; i < users.length; i++) {
		...
		var html = '<li tag_user="'+users[i]+'">'+users[i]+'</li>';
		//alert(html);
		$('[name=class_users]').append(html);
	}
};
...
function addUser(user) {
	...
	slElement.text(user);
	$("#usersList").append(slElement);
	var html = '<li tag_user="'+user+'">'+user+'</li>';
	//alert(html);
	$('[name=class_users]').append(html);
};
...
function removeUser(user) {
	...
	$('[name=class_users] li').each(function(){
		if($(this).attr("tag_user") == user){
			$(this).remove();
		}
		//$(this).attr("tat_user");
	});
